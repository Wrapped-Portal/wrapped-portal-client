...
import { Button, Card, Image, Input, MultiSelect, Slider, Text } from '@mantine/core';
    ...
    <div className='rec'>
      ...
      <h4>Enter an Artist to Base your Recommendations On</h4>
        ...
        {selectedOption.includes('Genre') && (
          <>
          <h4>Choose up to Five Genres seperated by a comma</h4>
          <Input
            placeholder="Soul,Trap,Shoegaze,Ambient,Techno"
            radius="xl"
        ...
        {selectedOption.includes('Danceability') && (
          ...
          color="pink"
            radius="xs"
            marks={[{ value: 0, label: 'No Danceability' }, { value: 50, label: 'Average Danceability' }, { value: 100, label: 'Extreme Danceability' },]}
        ...
        {selectedOption.includes('Energy') && (
          ...
          color="lime"
            radius="xs"
            marks={[{ value: 0, label: 'Low Enegry' }, { value: 50, label: 'Average Energy' }, { value: 100, label: 'Very Enegetic' },]}
        ...
        {selectedOption.includes('Loudness') && (
          ...
          color="violet"
            radius="xs"
            marks={[{ value: 0, label: 'Very Quiet' }, { value: 50, label: 'Average' }, { value: 100, label: 'Very Loud' },]}
        ...
        {selectedOption.includes('Vibe') && (
          ...
          color="orange"
            radius="xs"
            marks={[{ value: 0, label: 'Sad/Angry' }, { value: 50, label: 'Indiffrent' }, { value: 100, label: 'Happy/Euphoric' },]}
        ...
        {selectedOption.includes('Tempo') && (
          ...
          color="red"
            radius="xs"
            marks={[{ value: 0, label: '60 BPM' }, { value: 50, label: '130 BPM' }, { value: 100, label: '200 BPM' },]}
        ...
        {selectedOption.includes('Popularity') && (
          ...
          label={null}
            radius="xs"
            color="yellow"
            marks={[{ value: 0, label: 'Obscure' }, { value: 50, label: 'Notable' }, { value: 100, label: 'Well Known' },]}
        ...
        {selectedOption.includes('Instrumentalness') && (
          ...
          color="teal"
            radius="xs"
            marks={[{ value: 0, label: 'Mostly Vocals' }, { value: 50, label: 'Half Vocals' }, { value: 100, label: 'No Vocals' },]}
  ...
  { data && Array.isArray(data.tracks) && data.tracks.length > 0 ? (
    data.tracks.map(item => (
      <Card shadow="sm" p="lg" radius="md" withBorder key={item.id}>
        ...
        <Card.Section>
        <Text weight={600}className='card-text-large'>{item.album.artists[0].name}</Text>
        </Card.Section>
        <Card.Section>
        <Text weight={300}className="card-text-small">{item.name}</Text>
        </Card.Section>
      </Card>
    ))
  ) : null }
